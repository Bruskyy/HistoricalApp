...
**Dentro (MVP — VISION §16.1, GDD):** Biblioteca com 1 Coleção de lançamento e ~5 jornadas guiadas por pergunta, formando um arco fechado dentro dela (LDD §1-2, §2.3 — estratégia de vertical slice: uma coleção primeiro, testada com usuários, antes de padronizar para as demais); Corredor do Tempo por jornada; lições + interações (LDD §3, §7); agendador SRS por afirmação com missões de revisão; Thoth com 3 estágios; streak compassivo com Selos do Tempo; XP/níveis/títulos (como hipóteses instrumentadas); nível de consenso; 2 cards compartilháveis; estatísticas essenciais; onboarding com vitória antes do cadastro; notificação diária única; ferramenta interna mínima de conteúdo; instrumentação e experimentos.
...
2. Fluxo: boas-vindas de Thoth (≤3 telas) → escolha de jornada (opções da coleção de lançamento, pela *pergunta central* de cada uma) → primeira lição → primeira celebração → convite de cadastro.
...
1. Timeline por jornada, navegável, com nós nos 5 estados visuais (apagado/tênue/aceso/chama firme/constelação) + estado "vacilando" (regressão).
2. Desbloqueio por jornada; Trilhas (LDD §2.2) mostram fios de conexão quando atravessam várias coleções.
3. Jornadas/coleções não cobertas pelo conteúdo de lançamento aparecem como "salas seladas" com gancho narrativo (GDD §4.0), sem interação além de teaser.
...
| **R1 — Piloto fechado** (coorte convidada) | RF-01→13 com as jornadas do vertical slice completas (LDD §2.3) | O1-O5 medidos; decisão registrada dos experimentos (XP, títulos); calibração do GDD Apêndice A revisada |
| **R2 — MVP público (lojas)** | ~5 jornadas de 1 coleção (LDD §2.3, vertical slice); naming test resolvido (VISION §19); ajustes do piloto | Metas O1-O2 confirmadas em coorte aberta |
...
| Escopo de conteúdo inflar com o pivô "Biblioteca de Alexandria" (uma coleção "completa" pode crescer bem além do necessário para validar a tese) | Lançamento segue a estratégia de vertical slice (LDD §2.3): ~5 jornadas de uma única coleção, formando um arco fechado, testadas com usuários antes de padronizar o formato para as demais; demais coleções ficam como mistério "em breve" (LDD §9.2, alerta de escopo; GDD §4.0) |
