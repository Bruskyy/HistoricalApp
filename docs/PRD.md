...
5. Conteúdo do módulo ativo disponível offline (download automático do próximo trecho em Wi-Fi); progresso sincroniza ao reconectar.
...
### RF-05 — Thoth (GDD §7)
1. 3 estágios de evolução com gatilhos de marco (1º módulo; 10 nós Dominados + 2º módulo), com celebração dedicada e card.
...
1. Painel com: nós por estado, precisão, streak, horas, módulos em andamento.
...
| RNF-2 | Offline-first no aprendizado | Lição do módulo ativo e revisões do dia funcionam sem rede (transporte público é o contexto primário da persona Marina); sincronização resiliente a conflito |
...
Abrir app → Thoth (2-3 telas) → escolher módulo pela pergunta central → lição 1 (gancho → narrativa → interações) → celebração (nó tênue → aceso) → convite de cadastro (recusável) → pedido de notificação → fecho com gancho do amanhã.
...
**H2 — Escolher minha pergunta** (todas) — *Quero escolher qual módulo começar pela pergunta que me intriga.*
✓ Dado o onboarding, quando vejo os módulos, então cada opção mostra sua pergunta central (não um título de "curso"); e posso trocar de módulo depois sem perder progresso nem sofrer punição.
...
✓ Dado que completo meu 1º módulo, então Thoth evolui ao estágio 2 com celebração e card; o pergaminho-diário dele mostra as eras que dominamos; nenhuma mensagem dele usa culpa (auditoria de copy).
...
✓ Dado que o módulo ativo foi baixado, quando estou offline, então lições e revisões do dia funcionam; ao reconectar, progresso sincroniza sem perda nem duplicação.
...
**H11 — Publicar conteúdo sem release** (equipe) — *Como editores, queremos publicar módulo novo sem atualizar o app.*
...
- **Árvore de apoio:** retenção D1/D7/D30 · sessões/semana · streak médio · acerto em revisão (80-90% saudável) · % módulos concluídos · momentos "aha" reportados (qualitativo) · compartilhamentos/usuário · conversão de cadastro pós-lição 1.
...
| Revisor especialista indisponível | Contratar/parceirizar antes da produção do módulo-piloto; sem revisor, sem publicação (LDD §9.1) |
