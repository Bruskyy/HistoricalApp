...
>
> **v6** — pivô de identidade do fundador: o Mneme não é um produto de cursos/jornadas — é **uma Biblioteca de Alexandria interativa**. A porta de entrada deixa de ser a Jornada (no sentido antigo, cross-era) e passa a ser a **Coleção** (o que uma pessoa realmente busca: "Império Romano", "Estoicismo", "Aristóteles"); dentro da coleção, o conteúdo se organiza por **grandes temas de compreensão** (módulos), não por lista cronológica exaustiva. Jornada não desaparece: vira o formato de consumo dentro de uma coleção, e o padrão cross-era que ela inaugurou sobrevive como **Trilha** — um formato secundário e deliberado, não mais a porta principal, agora também uma **experiência de desbloqueio**: terminar uma coleção revela a próxima trilha, montada com os módulos que já existem (seção 3, seção 7, seção 12; celebração de Coleção Completa detalhada no `GAME_DESIGN.md` §9.1).
>
> **v4 (segunda rodada, crítica do fundador à forma do conteúdo)** — o que a v6 chamou de "módulo" vira **Jornada**: uma pergunta com começo, clímax e fim ("Como Roma deixou de ser uma República?"), não um capítulo de livro. É uma reutilização deliberada da palavra "Jornada" com um significado novo — o significado antigo (arco cross-era) já tinha virado **Trilha** na v6, então não há colisão: Jornada = unidade primária dentro de uma Coleção; Trilha = formato secundário que reaproveita nós de várias Jornadas/Coleções. Coleções de lançamento visam ~5 Jornadas formando um arco fechado, não uma lista aberta (LDD §2.3, estratégia de vertical slice).
...
Ninguém pensa "qual jornada do Mneme fala sobre a Revolução Francesa?" — pensa "quero entender a Revolução Francesa". O produto entra pela porta que a pessoa já busca: a **Coleção** (Império Romano, Primeira Guerra Mundial, Estoicismo, Aristóteles) — o **conteúdo**, o que existe para aprender. A partir daí, o Mneme conduz — nunca despeja: dentro da coleção, o conteúdo é organizado por **grandes perguntas de compreensão** (jornadas — LDD v4) — a **forma de aprender**, com apenas os acontecimentos indispensáveis para responder cada pergunta, não uma lista exaustiva de datas. Primeiro o conceito, depois os acontecimentos que o sustentam — nunca o contrário. Essa distinção (conteúdo vs. forma de aprender) é o que torna o produto uma plataforma: as mesmas coleções e jornadas, os mesmos "blocos", alimentam a Biblioteca, o Corredor, as Trilhas e qualquer futura forma de explorar o Arquivo — em vez de cada experiência exigir conteúdo próprio.

Isso não abandona a timeline — ela migra de escopo. O **Corredor do Tempo** continua sendo a recompensa visual e a estrutura de navegação de *cada* coleção (o usuário caminha as jornadas e salões de "Império Romano" como sempre caminhou um percurso), só deixa de ser uma única linha cronológica global da humanidade que o produto tenta preencher inteira. Onde o Duolingo tem "o caminho", nós temos **a coleção que você entrou para entender** — e o Corredor é como ela se vê e se sente.
...
- **Frente à Wikipédia/enciclopédias (referência exaustiva):** a Wikipédia entrega tudo sobre um assunto, sem hierarquia de importância nem narrativa — cabe ao leitor montar sentido sozinho. O Mneme cura: dentro de "Império Romano", só as jornadas e nós indispensáveis para responder às grandes perguntas de compreensão, numa ordem que ensina. Menos conteúdo, mais entendimento.
...
> O detalhamento operacional destes princípios (como nasce uma coleção, anatomia de uma lição, o que conta como "aprendido", como escrever perguntas de compreensão) vive no [`LEARNING_DESIGN.md`](LEARNING_DESIGN.md) — etapa nova do roadmap, anterior ao GDD.
...
- **Coleção, não curso** *(v6, retitulado v4)* — a porta de entrada é "Império Romano", não "Curso de História Antiga"; dentro dela, jornadas guiadas por pergunta ("Como Roma deixou de ser uma República?", "Como Roma manteve a paz por 200 anos?"...), cada uma com só os nós indispensáveis para responder à pergunta (LDD §2). O padrão cross-era que definia "jornada" nas versões anteriores da visão — "Grécia → Sócrates → Platão → Roma → Iluminismo → Revolução Francesa → Brasil" — sobrevive como **Trilha**: um formato secundário e deliberado para quando a pergunta central só existe *entre* coleções (LDD §2.2), montado reaproveitando jornadas que já existem (nenhuma lição nova) e descoberto via Ecos, via desbloqueio ao concluir uma coleção, ou numa prateleira própria da Biblioteca — nunca a porta principal.
...
2. 1 Coleção de lançamento com ~5 jornadas guiadas por pergunta, formando um arco fechado (não uma lista aberta), produzidas pelo pipeline do LDD (§1-2) — validadas com usuários reais antes de padronizar o formato para as demais coleções (LDD §2.3, estratégia de vertical slice).
